...
// +kubebuilder:validation:XValidation:rule="(has(self.existingClaimName) && self.existingClaimName != '') != (has(self.persistentVolumeClaim) && size(self.persistentVolumeClaim.accessModes) > 0)",message="set exactly one of storage.existingClaimName or storage.persistentVolumeClaim"
